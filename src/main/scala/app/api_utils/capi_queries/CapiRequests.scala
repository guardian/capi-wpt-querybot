package app.api_utils.capi_queries
...
case class ContentPage(fields: Option[ContentFields], tags: Seq[Tag], webUrl: String, creatorEmail: Option[String])

class CapiRequests(key: String) {
...
*/
  ...
  def getUrlsForContentType(contentType: String): List[ContentPage] = {
     contentType match {
  ...
  def getArticles(pageNumber: Int): List[ContentPage] = {
    ...
    try {
      ...
      val articleContentAndUrl: List[ContentPage] = for (result <- returnedResponse.results.toList) yield {
        ContentPage(result.fields, result.tags, result.webUrl, None)
      }
      ...
      case _: Throwable => println("bad request - page is empty - returning empty list")
        List.empty[ContentPage]
    }
  }

  def getMinByMins(pageNumber: Int): List[ContentPage] = {
  try {
    val searchQuery = SearchQuery()
      .fromDate(from)
      .toDate(until)
    ...
    val apiResponse = contentApiClient.getResponse(searchQuery)
    val returnedResponse = Await.result(apiResponse, (20, SECONDS))
    val liveBlogContentAndUrl: List[ContentPage] = for (result <- returnedResponse.results.toList) yield {
        ContentPage(result.fields, result.tags, result.webUrl, None)
      }
  ...
  def getInteractives(pageNumber: Int): List[ContentPage] = {
  try {
    val searchQuery = SearchQuery()
      .fromDate(from)
      .toDate(until)
    ...
    val apiResponse = contentApiClient.getResponse(searchQuery)
    val returnedResponse = Await.result(apiResponse, (20, SECONDS))
    val interactiveContentAndUrl: List[ContentPage] = for (result <- returnedResponse.results.toList) yield {
        val creatorEmail = result.blocks.flatMap(_.body).map(_.map(_.createdBy.map(_.email).getOrElse("")).head)
        ContentPage(result.fields, result.tags, result.webUrl, creatorEmail)
    }
  ...
  def getFronts(pageNumber: Int): List[ContentPage] = {
    ...
    val returnList:List[ContentPage] = listofFronts.map(url => ContentPage(emptyContentFields, emptyTags, url, None))
    println("CAPI Query Success - Fronts: \n" + returnList.map(element => element.webUrl).mkString)
    returnList
  }


  def getVideoPages(pageNumber: Int): List[ContentPage] = {
  try {
    val searchQuery = SearchQuery()
      .fromDate(from)
      .toDate(until)
    ...
    val apiResponse = contentApiClient.getResponse(searchQuery)
    val returnedResponse = Await.result(apiResponse, (20, SECONDS))
    val videoContentAndUrl: List[ContentPage] = for (result <- returnedResponse.results.toList) yield {
        ContentPage(result.fields, result.tags, result.webUrl, None)
      }
  ...
  def getAudioPages(pageNumber: Int): List[ContentPage] = {
    try {
      ...
      val returnedResponse = Await.result(apiResponse, (20, SECONDS))
      val audioContentAndUrl: List[ContentPage] = for (result <- returnedResponse.results.toList) yield {
          ContentPage(result.fields, result.tags, result.webUrl, None)
        }
  ...
  def getSinglePage(urlString: String): ContentPage = {
    ...
    try {
      ...
      val resultAndUrl: ContentPage = {
        val returnTuple = for (content <- returnedResponse.content) yield {
          ...
          val returnFields = ContentPage(contentFields, content.tags, content.webUrl, creatorEmail)
          returnFields
        }
        returnTuple.getOrElse(ContentPage(Option(makeContentStub(Option(urlString), None, Option(false))), emptyTags, urlString, None))
      }

      if(resultAndUrl.fields.isEmpty){
        println("result and url is coming back as Empty")
      } else {
        println("CAPI headline result: \n" + resultAndUrl.fields.get.headline)
      }
      resultAndUrl
    } catch {
      case _: Throwable => println("bad request - page is empty - returning empty content fields")
        val emptyTags: Seq[Tag] = Seq()
        val pageWithEmptyContentFields: ContentPage = ContentPage(Option(makeContentStub(Option(urlString), None, Option(false))), emptyTags, urlString, None)
        pageWithEmptyContentFields
    }
  }
