package app.api_utils.previous_results
...
class ResultsFromPreviousTests(resultsList: List[PerformanceResultsObject]) {
  ...
  def returnPagesNotYetTested(list: List[ContentPage]): List[ContentPage] = {
    val pagesNotYetTested: List[ContentPage] = for (page <- list if !previousResults.map(_.testUrl).contains(page._3)) yield page
    val pagesAlreadyTested:List[ContentPage] = for (page <- list if previousResults.map(_.testUrl).contains(page._3)) yield page
