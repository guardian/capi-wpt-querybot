...
//    val frontsUrls: List[String] = for (page <- fronts) yield page._3
    ...
    val previousArticlesToRetest: List[PerformanceResultsObject] = previousResultsToRetest.filter(_.getPageType.contains("Article"))
    val previousLiveBlogsToRetest: List[PerformanceResultsObject] = previousResultsToRetest.filter(_.getPageType.contains("LiveBlog"))
    val previousInteractivesToRetest: List[PerformanceResultsObject] = previousResultsToRetest.filter(_.getPageType.contains("Interactive"))
    ...
    if (combinedArticleList.nonEmpty) {
      ...
      val articleAverages: PageAverageObject = new ArticleDefaultAverages()
      //listen for results from web page test:
      ...
      if (sortedByWeightArticleResultsList.isEmpty) {
    ...
    if (combinedInteractiveList.nonEmpty) {
      ...
      val interactiveAverages: PageAverageObject = new InteractiveDefaultAverages()
